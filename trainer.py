...
class Trainer(object):
    ...
    def test(self):
        ...
        for i in range(self.args.test_size):
            attr_img, id_img, id_mask = self.data_loader.get_batch(is_train=False, is_cross=False)

            pred, id_embedding, w, attr_embedding, src_lnds = self.model.G(id_mask, attr_img, id_img)
            ...
            if self.args.test_with_arcface:
                try:
                    arc_id_embedding = self.model.G.test_id_encoder(id_mask)
            ...
            # Fake Reconstruction
            self.test_reconstruction(id_mask, fake_reconstruction, display=(i==0), display_name='id_img')
            ...
            if i == 0:
                utils.save_image(image[0], self.args.images_results.joinpath(f'test_{self.num_epoch}_prediction.png'))
                utils.save_image(id_img[0], self.args.images_results.joinpath(f'test_{self.num_epoch}_gt.png'))
                utils.save_image(attr_img[0],self.args.images_results.joinpath(f'test_{self.num_epoch}_attr.png'))
                utils.save_image(id_mask[0], self.args.images_results.joinpath(f'test_{self.num_epoch}_id.png'))

                Writer.add_image('test/prediction', image, step=self.num_epoch)
                Writer.add_image('test input/gt image', id_img, step=self.num_epoch)
                Writer.add_image('test input/attr image', attr_img, step=self.num_epoch)
                Writer.add_image('test input/id image', id_mask, step=self.num_epoch)
